    ...
    for pdf_file in pdf_files:
        ...
        try:
            ...
            try:
                ...
                try:
                    subprocess.run(
                        [sys.executable, "-m", "pip", "install", "PyPDF2"], check=True
                    )
                    import PyPDF2
...
"
                ...
                print(
                    "Processed {} with PyPDF2, extracted {} paragraphs".format(
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        print(
                            "Warning: Could not install poppler with brew: {}".format(
                                brew_error
                            )
                        )
                        raise Exception(
                            "pdftotext not available and could not be installed"
                        )

                # Use pdftotext
                text_output = os.path.join(output_dir, "{}.txt".format(pdf_file))
                subprocess.run(
                    ["pdftotext", "-layout", pdf_path, text_output], check=True
                )
