...
async def mineru_parse_async(
    session, file_path, server_url="http://127.0.0.1:8000/predict", **options
):
    """
    ...
    try:
        ...
        payload = {"file": file_b64, "options": options}
        ...
        async with session.post(server_url, json=payload) as response:
            if response.status == 200:
                ...
                logger.info(
                    f"✅ Processed: {file_path} -> {result.get('output_dir', 'N/A')}"
                )
                return result
            else:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        # === Basic Processing ===
        basic_tasks = [
            mineru_parse_async(session, file_path) for file_path in existing_files[:2]
        ]
        ...
        custom_tasks = [
            mineru_parse_async(session, file_path, **custom_options)
