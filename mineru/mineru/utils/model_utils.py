...
def crop_img(input_res, input_img, crop_paste_x=0, crop_paste_y=0):
    ...
    if isinstance(input_img, np.ndarray):
        ...
        return_image = (
            np.ones((crop_new_height, crop_new_width, 3), dtype=np.uint8) * 255
        )
...
def calculate_iou(box1, box2):
    ...
    intersection_xmin, intersection_ymin, intersection_xmax, intersection_ymax = (
        intersection
    )
        ...
        while i < len(table_res_list) - 1:
            ...
            while j < len(table_res_list):
                ...
                if iou > iou_threshold:
                        ...
                        (
                            ...
                            if k < min(to_remove)
                                ...
                                if k < max(to_remove)
                                    ...
                                    k - 2 if k > max(to_remove) else len(layout_res) - 1
                                )
                            )
                        )
                    ...
                    # Update table_info
                    table_info = [
                        get_coords_and_area(table) for table in table_res_list
                    ]
...
def filter_nested_tables(table_res_list, overlap_threshold=0.8, area_threshold=0.8):
    ...
    for i in range(len(table_res_list)):
        ...
        if len(tables_inside) >= 3:
            ...
            tables_overlap = any(
                do_overlap(
                    table_info[tables_inside[idx1]], table_info[tables_inside[idx2]]
                )
...
def get_res_list_from_layout_res(
    layout_res, iou_threshold=0.7, overlap_threshold=0.8, area_threshold=0.8
):
    ...
    if len(filtered_table_res_list) < len(table_res_list):
        ...
        to_remove = [
            table_indices[i]
            for i, table in enumerate(table_res_list)
