...
def do_predict(boxes: List[List[int]], model) -> List[int]:
    from mineru.model.reading_order.layout_reader import (boxes2inputs,
                                                          parse_logits,
                                                          prepare_inputs)
...
def cal_block_index(fix_blocks, sorted_bboxes):
    ...
    if sorted_bboxes is not None:
        ...
        res = []
        recursive_xy_cut(
            np.asarray(random_boxes).astype(int), np.arange(len(block_bboxes)), res
        )
...
def revert_group_blocks(blocks):
    ...
    for group_id, blocks in image_groups.items():
        new_blocks.append(
            process_block_list(blocks, BlockType.IMAGE_BODY, BlockType.IMAGE)
        )

    for group_id, blocks in table_groups.items():
        new_blocks.append(
            process_block_list(blocks, BlockType.TABLE_BODY, BlockType.TABLE)
        )
