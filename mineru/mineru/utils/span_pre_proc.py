...
def __replace_ligatures(text: str):
    ...
    return re.sub(
        "|".join(map(re.escape, ligatures.keys())), lambda m: ligatures[m.group()], text
    )
...
def txt_spans_extract(
    pdf_page, spans, pil_img, scale, all_bboxes, all_discarded_blocks
):
        ...
        if span["type"] in [ContentType.TEXT]:
            for block in all_bboxes + all_discarded_blocks:
                if block[7] in [
                    ...
                    continue
                if (
                    calculate_overlap_area_in_bbox1_area_ratio(span["bbox"], block[0:4])
                    > 0.5
                ):
                    if (
                        span["height"] > median_span_height * 3
                        and span["height"] > span["width"] * 3
                    ):
    ...
    if len(vertical_spans) > 0:
        for pdfium_line in page_all_lines:
            for span in vertical_spans:
                if (
                    calculate_overlap_area_in_bbox1_area_ratio(
                        pdfium_line["bbox"].bbox, span["bbox"]
                    )
                    > 0.5
                ):
...
def calculate_char_in_span(
    char_bbox, span_bbox, char, span_height_radio=Span_Height_Radio
):
    ...
    if (
        ...
        < span_height
        * span_height_radio  # 字符的中轴和span的中轴高度差不能超过Span_Height_Radio
    ):
        return True
    else:
...
def chars_to_content(span):
    ...
    if len(span["chars"]) == 0:
        ...
        for char in span["chars"]:
            ...
            char1 = char
            char2 = (
                span["chars"][span["chars"].index(char) + 1]
                if span["chars"].index(char) + 1 < len(span["chars"])
                else None
            )
            if (
                char2
