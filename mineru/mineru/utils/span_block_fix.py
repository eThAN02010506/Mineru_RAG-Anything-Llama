...
from mineru.utils.ocr_utils import (_is_overlaps_x_exceeds_threshold,
...
def fill_spans_in_blocks(blocks, spans, radio):
    ...
    for block in blocks:
        ...
        for span in spans:
            ...
            ) > radio and span_block_type_compatible(span["type"], block_type):
...
def fix_text_block(block):
    ...
    vertical_span_count = sum(
        1
        for span in block["spans"]
        if (span["bbox"][3] - span["bbox"][1]) / (span["bbox"][2] - span["bbox"][0])
...
def merge_spans_to_vertical_line(spans, threshold=0.6):
    ...
    if len(spans) == 0:
        ...
        for span in spans[1:]:
            ...
            # 如果当前的span与当前行的最后一个span在y轴上重叠，则添加到当前行
            if _is_overlaps_x_exceeds_threshold(
                span["bbox"], current_line[-1]["bbox"], threshold
            ):
                current_line.append(span)
            else:
