...
from mineru.utils.span_block_fix import (fill_spans_in_blocks, fix_block_spans,
                                         fix_discarded_block)
from mineru.utils.span_pre_proc import (remove_outside_spans,
...
def page_model_info_to_page_info(
    ...
    if len(maybe_text_image_blocks) > 0:
        for block in maybe_text_image_blocks:
            ...
            for span in spans:
                if (
                    span["type"] == "text"
                    and calculate_overlap_area_in_bbox1_area_ratio(
            ...
            if len(span_in_block_list) > 0:
                ...
                block_area = (block["bbox"][2] - block["bbox"][0]) * (
    ...
    page_info = make_page_info_dict(
        sorted_blocks, page_index, page_w, page_h, fix_discarded_blocks
    )
...
def result_to_middle_json(
    ...
    for page_index, page_model_info in tqdm(
        enumerate(model_list), total=len(model_list), desc="Processing pages"
    ):
    ...
    if len(img_crop_list) > 0:
        ...
        ocr_model = atom_model_manager.get_atom_model(
            atom_model_name="ocr", ocr_show_log=False, det_db_box_thresh=0.3, lang=lang
        )
