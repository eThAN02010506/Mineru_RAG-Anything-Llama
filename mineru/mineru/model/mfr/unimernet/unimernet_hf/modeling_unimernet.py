...
from .unimer_swin import (UnimerSwinConfig, UnimerSwinImageProcessor,
                          UnimerSwinModel)
...
def fix_left_right_pairs(latex_formula):
    ...
    while i < len(latex_formula):
        ...
        # 检测\left命令
        if (
            i + 5 < len(latex_formula)
            and latex_formula[i : i + 5] == "\\left"
            and i + 5 < len(latex_formula)
        ):
        ...
        # 检测\right命令
            ...
            i + 6 < len(latex_formula)
            and latex_formula[i : i + 6] == "\\right"
            and i + 6 < len(latex_formula)
        ):
...
def process_latex(input_string):
    ...
    def replace_func(match):
        ...
        if "a" <= next_char <= "z" or "A" <= next_char <= "Z":
            ...
            if pos < len(input_string) and (
                "a" <= input_string[pos] <= "z" or "A" <= input_string[pos] <= "Z"
            ):
                # 下一个字符也是字母，保持不变
                return match.group(0)
...
ENV_FORMAT_PATTERNS = {
    env: re.compile(r"\\begin\{" + env + r"\}\{([^}]*)\}") for env in ENV_TYPES
}
...
def latex_rm_whitespace(s: str):
    ...
    s = UP_PATTERN.sub(
        lambda m: (
            m.group(0)
            if m.group(1) in ["arrow", "downarrow", "lus", "silon"]
            else f"\\{m.group(1)}"
        ),
        s,
    )
...
class UnimernetModel(VisionEncoderDecoderModel):
    ...
    def from_checkpoint(
        cls,
        model_path: str,
        model_filename: str = "pytorch_model.pth",
        ...
        if state_dict_strip_prefix:
            state_dict = {
                (
                    k[len(state_dict_strip_prefix) :]
                    if k.startswith(state_dict_strip_prefix)
                    else k
                ): v
