...
class UnimerSwinEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if self.position_embeddings is not None:
            ...
            embeddings = (
                embeddings + self.position_embeddings[:, :seq_len, :]
            )  # code edited.
...
class UnimerSwinLayer(nn.Module):
    def __init__(self, config, dim, input_resolution, num_heads, shift_size=0):
        ...
        self.ce = nn.ModuleList(
            [ConvEnhance(config, dim=dim, k=3), ConvEnhance(config, dim=dim, k=3)]
        )

        self.attention = UnimerSwinAttention(
            config, dim, num_heads, window_size=self.window_size
        )
        self.drop_path = (
            UnimerSwinDropPath(config.drop_path_rate)
