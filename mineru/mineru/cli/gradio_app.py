...
header_path = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "resources", "header.html"
)
    ...
    **kwargs,
    ...
    if sglang_engine_enable:
        try:
            ...
            predictor = model_singleton.get_model("sglang-engine", None, None, **kwargs)
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column(variant="panel", scale=5):
                with gr.Row():
                    input_file = gr.File(
                        label="Please upload a PDF or image", file_types=suffixes
                    )
                with gr.Row():
                    max_pages = gr.Slider(
                        1,
                        max_convert_pages,
                        int(max_convert_pages / 2),
                        step=1,
                ...
                with gr.Row():
                    ...
                    backend = gr.Dropdown(
                        drop_list, label="Backend", value=preferred_option
                    )
                ...
                with gr.Row(equal_height=True):
                    with gr.Column():
                        ...
                        table_enable = gr.Checkbox(
                            label="Enable table recognition", value=True
                        )
                ...
                pdf_show = PDF(
                    label="PDF preview", interactive=False, visible=True, height=800
                )
                if example_enable:
        ...
        input_file.change(
            fn=to_pdf, inputs=input_file, outputs=pdf_show, api_name=api_name
        )
        change_bu.click(
            fn=to_markdown,
            inputs=[
