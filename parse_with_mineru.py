        ...
        if mineru_available:
            try:
                ...
                if os.path.exists(mineru_output_dir):
                    ...
                    content_list_json = os.path.join(mineru_output_dir, "{}_content_list.json".format(pdf_name))
                    middle_json = os.path.join(mineru_output_dir, "{}_middle.json".format(pdf_name))
                    model_json = os.path.join(mineru_output_dir, "{}_model.json".format(pdf_name))
...
#{1,6} ', md_text)
                        ...
                        json_files = glob.glob(os.path.join(mineru_output_dir, "*.json"))
                        if json_files:
                            ...
                            for json_file in json_files:
                                try:
                                    ...
                                    print("Error processing JSON file {}: {}".format(json_file, e))
                        else:
                            print("Warning: No output files found in MinerU output directory")
                            raise Exception("No output files found")
                else:
                    print("Warning: MinerU output directory not found: {}".format(mineru_output_dir))
                    print("Available directories in {}: {}".format(output_dir, os.listdir(output_dir)))
                    
                    # Try to find any directory that might contain MinerU output
                    pdf_name_dirs = [d for d in os.listdir(output_dir) if os.path.isdir(os.path.join(output_dir, d))]
                    if pdf_name_dirs:
                        print("Found potential PDF name directories: {}".format(pdf_name_dirs))
                        for pdf_dir in pdf_name_dirs:
                            auto_dir = os.path.join(output_dir, pdf_dir, "auto")
                            if os.path.exists(auto_dir):
                                print("Found potential MinerU output directory: {}".format(auto_dir))
                                ...
                                if json_files or md_files:
                                    print("Found files in {}: {} JSON files, {} MD files".format(
...
#{1,6} ', md_text)
                                            ...
                                            print("Error processing MD file {}: {}".format(md_file, e))
...
") if p.strip()]
                        ...
                        print("Processed {} with pdftotext, extracted {} paragraphs".format(
                        ...
                        try:
                            ...
                            with open(pdf_path, 'rb') as pdf_file_obj:
                                ...
                                print("Processed {} with PyPDF2, extracted {} pages".format(
                                    pdf_file, len(pdf_reader.pages)))
                        except Exception as pypdf_error:
                            print("Error: All extraction methods failed for {}: {}".format(
                    ...
                    print("Error: Fallback processing failed: {}".format(fallback_error))
